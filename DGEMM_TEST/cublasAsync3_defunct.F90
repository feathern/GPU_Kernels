PROGRAM MAIN
    ...
    Integer(4):: rc, rc2,nt, ntheta, nmatrices, iter,i, nvec, ntests
...
!    Character*1 :: transa='T', transb='N'
    ...
    Do iter = 1, ntests
        ...
        Do i = 1, nmatrices,2
            ...
            queue = mod(i,numHandles) + 1
            !$ACC update device(lhs(i:i+1)%vals) async(queue)
            ...
            ndim2 = i
            ndim3 = ntheta-i+2
            !!$ACC data present(mmatrix,mmatrix2,lh,rh,lh2,rh2)
            ...
            !rc = cublasDGEMM_v2(cuHandles(queue),transa,transb,ndim3,nvec,ntheta, alpha, mmatrix, &
            !    &  ntheta,lh2 , ntheta, beta,rh2,ndim3)
            !$ACC END DATA
            !Write(6,*) "RH=", rh(1,1)

            !$ACC update host(rhs(i:i+1)%vals) async(queue)
        Enddo
        !$ACC WAIT 
        !//////////////////////
    Enddo
