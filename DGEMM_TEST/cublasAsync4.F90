...
!    Character*1 :: transa='T', transb='N'
    ...
    Do iter = 1, ntests
        ...
        Do i = 1, nmatrices,2
            ...
            !$ACC update device(lhs(i)%vals) async(queue)
            !if (MOD(i,2) .eq. 0) Then
            !    ndim2 = ntheta -i+1
            !Else
            !    ndim2 = i
            ...
            !$ACC END DATA
            !$ACC update host(rhs(i)%vals) async(queue)

            ndim3 = ntheta-i+2

            !$ACC update device(lhs(i+1)%vals) async(queue)
            !$ACC data present(mmatrix2,lh2,rh2)
            rc = cublasDGEMM_v2(cuHandles(queue),transa,transb,ndim3,nvec,ntheta, alpha, mmatrix2, &
                &  ntheta,lh2 , ntheta, beta,rh2,ndim3)
            !$ACC END DATA

            !$ACC update host(rhs(i+1)%vals) async(queue)
        Enddo
        !$ACC WAIT 
        !//////////////////////
    Enddo
